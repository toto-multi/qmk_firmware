...
const PROGMEM uint16_t KANA_TABLE[][2][KANA_MAX] = {
  ...
  { { JP_SLSH ,XXXXXXX ,XXXXXXX }, { JP_ASTR ,XXXXXXX ,XXXXXXX } },
  // 右手 上段
  { { JP_COLN ,XXXXXXX ,XXXXXXX }, { XXXXXXX ,XXXXXXX ,XXXXXXX } },
...
const PROGMEM SEIDAKU SEIDAKU_TABEL[] = {
...
// 拗音・外来音テーブル
const PROGMEM uint16_t COMB_TABEL[][2][KANA_MAX] = {
...
bool dakuon( uint16_t kana[KANA_MAX] ) {
  ...
  SEIDAKU seidaku;
  for ( int i = 0; i < ( sizeof( SEIDAKU_TABEL ) / sizeof( seidaku ) ); i++ ) {
    memcpy_P( &seidaku, &SEIDAKU_TABEL[i], sizeof( seidaku ) );
...
void combine_kana( uint16_t kana_0[2][KANA_MAX], uint16_t kana_1[2][KANA_MAX] ) {
  ...
  for ( int i = 0; i < 2; i++ ) {
    for ( int j = 0; j < 2; j++ ) {
      for ( int k = 0; k < ( sizeof( COMB_TABEL ) / sizeof( comb ) ); k++ ) {
        memcpy_P(&comb, &COMB_TABEL[k], sizeof( comb ) );
